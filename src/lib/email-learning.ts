...
/** Fetch the stored voice prompt from admin_settings (generated by learn-voice agent). */
async function getVoicePrompt(): Promise<string | null> {
  try {
    const supabase = requireSupabaseAdmin();
    const { data } = await supabase
      .from("admin_settings")
...
export async function buildLearningContext(): Promise<string> {
  const [responses, voicePrompt] = await Promise.all([
    getRecentResponses(15).catch(() => []),
    getVoicePrompt(),
  ]);

  let context = "";

  // Voice prompt from the learn-voice agent (highest priority — extracted from real Gmail)
  if (voicePrompt) {
    context += `\n# Krishna's communication voice (extracted from his real sent emails)\n${voicePrompt}\n`;
  }

  const sentResponses = responses.filter((r) => r.action !== "discarded");
  if (sentResponses.length === 0) return context;
  ...
  context += `\n# Recent email patterns (${sentResponses.length} tracked responses)\n`;
