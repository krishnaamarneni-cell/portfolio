...
async function fetchViaApi(keyword: string): Promise<JobDivaListing[]> {
  try {
    ...
    return jobs.map((j: Record<string, unknown>) => ({
      ...
      url: `${PORTAL_BASE}#/job/${j.jobId || j.id || ""}`,
