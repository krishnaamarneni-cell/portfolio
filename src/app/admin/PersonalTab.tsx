...
function RecruiterContactsCard({
  onError,
  onSuccess,
}: {
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-[#1a1a1a] overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0 text-left">
          <h3 className="font-bold text-white">
            Recruiter Contacts{" "}
            <span className="text-[10px] font-mono text-[#666] ml-1">
              {contacts.length}
            </span>
          </h3>
          <p className="text-[11px] text-[#666]">
            Extracted from your inbox by Email Intelligence. Send outreach with one tap.
          </p>
        </div>
        <span className="text-[10px] font-mono text-[#666]">
          {open ? "HIDE" : "OPEN"}
      ...
      {open && (
        <div className="px-5 pb-5 space-y-4">
          {contacts.length === 0 && (
            <p className="text-xs text-[#666]">
              No contacts yet. Run the Email Intelligence agent in the Agents tab to extract recruiter contacts from your inbox.
            </p>
          )}
    ...
    <div className="flex items-center gap-3 p-3 rounded-xl bg-[#0a0a0a] border border-white/[0.05]">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-white truncate">
          {c.name || c.email}
          {c.company && (
            <span className="text-[#666] ml-1">@ {c.company}</span>
          )}
        </p>
        <p className="text-[10px] text-[#666] truncate">
          {c.email}
